...
export default function(){
    ...
    const [comapny_location, setLocation] = useState("");
    ...
    return(
        <div className="bg-slate-700 h-screen w-screen flex justify-center items-center">
            ...
            <div className=" rounded-md flex flex-col bg-white h-[50vh] items-center justify-between text-bolded text-black w-1/3 p-4" >
                ...
                <input type = "text" placeholder="company location" className="border-2 rounded-md h-1/6 w-1/2 p-4" onChange={(e)=>{setLocation(e.target.value)}}></input>
                ...
                <button className="bg-blue-500 w-1/3 rounded-md h-1/6" onClick={async ()=>{
                    await axios.post('http://localhost:3000/api/company', {email, company_name, password, comnpany_wallet, comapny_location, comapny_number})
                }}>Sign Up</button>
            </div>
            </div>
        </div>
